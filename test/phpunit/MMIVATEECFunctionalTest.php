...
/**
 * \file    test/phpunit/MMIVATEECFunctionalTest.php
 * \ingroup mmivateec
 ...
 *
 * @package Testmmivateec
 */
class MMIVATEECFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMIVATEEC')->click();
		} else {
			// Disable the module
			$this->byHref('modMMIVATEEC')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmivateec/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmivateec/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmivateec/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmivateec/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmivateecwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMIVATEEC_MMIVATEECTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMIVATEEC_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
